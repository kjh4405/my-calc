...
dup = st.sidebar.radio("하위 복제 인원 (2~6대)", [2, 3], index=0) # 2명/3명 선택 복구
...
total_expense = init_cost + monthly_exp # 총 지출 (초기비용 + 월지출)

# B. 수익 계산 (유니레벨 + 바이너리 + 오빗 모두 포함)
...
with tabs[3]:
    ...
    st.write(f"- 초기 비용 합계 (패키지+회비): ${init_cost:,}")
