...
# --- 2. 6개 국어 사전 (일본어 번역 및 ADIL 로직 보존) ---
...
base_sub = (my_gc / 120) * 72.0  # 기본 120게임/72달러 지불

# 72 CV 유지 조건 로직
my_earned_cv = my_gc * 20 * pkgs[my_p]["self_rate"] # 본인 적립 CV
cv_short = max(0.0, 72.0 - my_earned_cv)

# Basic/Standard는 부족분을 추가 비용($)으로 채움, Premium/Ultimate는 이미 충족됨
...
with st.expander("ℹ️ 참고용 비용 안내 (Reference Cost Info)"):
    ...
    with col_info2:
        st.write(f"**🔹 월간 예상 유지 비용:** `${month_exp:,.1f}`")
        if my_p in ["Basic", "Standard"]:
            st.caption(f"(구독료 $72 + 72 CV 유지를 위한 부족분 보충 비용 포함)")
            st.info(f"💡 {my_p} 등급은 120게임 플레이와 $72 지불로 72 CV를 유지하는 구조입니다.")
        else:
            st.caption(f"(120게임 플레이로 72 CV 자동 충당)")
            st.success(f"✨ {my_p} 등급은 높은 적립률로 인해 120게임 플레이만으로 유지 조건이 충족됩니다.")
