...
st.title("🚀 DHP 비지니스 종합 수익 분석 (정밀 복구본)")

# 1. 데이터 정의 (가장 정확했던 로직 그대로)
pkgs = {
...
# 지출 합계 (순수익 계산용)
total_expense_for_net = init_cost + monthly_exp
...
# C. 순수익 계산 (사용자 요청 공식 반영)
# (총 등록 보너스 + 월 연금 수익) - (초기 비용 + 월 지출)
net_profit = (total_reg_bonus + total_mon_bonus) - total_expense_for_net
...
with tabs[3]:
    ...
    st.markdown(f"### **종합 지출액: ${total_expense_for_net:,.2f}**")
