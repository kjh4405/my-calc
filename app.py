...
if lang == "Korean":
    t = {
        ...
        "adil_title": "🪙 ADIL 코인 가치 분석", "listing": "ADIL 시세", "prob": "1위 당첨 기대가치",
    ...
    t = {
        ...
        "adil_title": "🪙 ADIL Token Analysis", "listing": "ADIL Price", "prob": "1st Place Expected Value",
    ...
    t = {
        ...
        "pa_p": "パートナーの等級", "l1": "1レベルの紹介人数", "dup": "複製人数 (2レベル以降)",
        "m1": "総組織人数", "m2": "初期費用", "m3": "月間支出", "m4": "登録ボーナス合計", "m5": "月間収入", "m6": "総合純利益",
        "tab1": "📊 ボーナス詳細", "tab2": "💰 ADIL期待収益", "tab3": "💳 支出詳細",
        "detail": "ボーナス詳細レポート", "item": "項目", "reg_s": "登録収入(単발)", "mon_s": "継続月間収入",
        "adil_title": "🪙 ADILトークン価値分析", "listing": "ADIL価格", "prob": "1位当選期待価値",
        "exp_title": "💳 支出詳細根拠", "init_h": "初期費用合計", "mon_h": "月間実支出", "total_h": "総合支出額"
    }
...
with tabs[1]:
    # ADIL 획득 로직 수정: 20달러 게임(120회) 기준 562.5개, 40달러 게임 시 두배
    game_unit = my_gc / 120
    adil_count = 562.5 * game_unit
    win_count = 7.5 * game_unit
    
    st.subheader(f"{t['adil_title']} (보유량: {adil_count:,.1f} ADIL)")
    st.info(f"💡 120게임당 {win_count:,.1f}회 1위 당첨 확률을 기반으로 한 가치 분석입니다.")
    
    adil_prices = [0.4, 0.5, 0.8, 1.0] # 요청하신 시세 리스트
    adil_results = []
    for p in adil_prices:
        total_value = adil_count * p
        expected_value = total_value + (win_count * p * 10) # 1위 당첨 시 가중치 예시 포함 가능
        adil_results.append({
            t["listing"]: f"${p}",
            "보유 가치": f"${total_value:,.1f}",
            t["prob"]: f"${expected_value:,.1f}"
        })
