...
t_all = {
    "Korean": {
        ...
        "ref_low_extra": "(구독료 $110.25 + 자격유지비 $72 합산)",
        "ref_low_ok": "(240게임 플레이로 추가비 없이 $110.25 유지)",
        "ref_high_ok": "(고정 유지비 $110.25로 모든 조건 충족)",
    ...
    "English": {
        ...
        "ref_low_extra": "($110.25 Sub + $72 Maintenance)",
        "ref_low_ok": "($110.25 Sub only - 240 games cover 72 CV)",
        "ref_high_ok": "($110.25 Fixed covers everything)",
...
needs_extra_72 = (my_p in ["Basic", "Standard"]) and (my_earned_cv < 72.0)
extra_fee = 72.0 if needs_extra_72 else 0.0
...
with st.expander(t["ref_title"]):
    ...
    with col_info2:
        ...
        if my_p in ["Basic", "Standard"]:
            if my_gc >= 240:
                st.caption(t["ref_low_ok"])
                st.success("✅ 240 games play: $72 fee waived.")
            else:
                st.caption(t["ref_low_extra"])
                st.warning("⚠️ 120 games play: $72 extra fee required.")
        else:
            st.caption(t["ref_high_ok"])
