...
# Basic/Standard 120게임 시 $72 추가 조건
...
with st.expander(t["ref_title"]):
    ...
    with col_info2:
        if extra_72 > 0:
            ...
            st.info(f"💡 {my_p} (120게임)는 자격 유지를 위해 $72 추가 지출이 필요합니다.")
        else:
            st.write(f"**{t['ref_month']}:** `${fixed_monthly_exp:,.2f}` ✅")
            if is_low_tier and not is_120_game:
                st.success(f"✨ {my_p} (240게임)는 자력으로 72 CV를 충족하여 $72 추가비용이 면제됩니다.")
...
with tab5:
    ...
    with c1:
        ...
        if extra_72 > 0:
            ...
            st.caption(f"({my_p} 120게임: 고정유지비 + 자격유지비)")
        else:
            st.write(f"**🟠 {t['exp_month']}:** `${fixed_monthly_exp:,.2f}`")
            if is_low_tier and not is_120_game:
                st.caption(f"(240게임 플레이로 추가비용 면제됨)")
